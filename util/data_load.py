...
class Data_load(torch.utils.data.Dataset):
    def __init__(self, img_root,mask_root, unwrap_root,img_transform, mask_transform):
        ...
        self.unwrap_paths = sorted(glob('{:s}/*.bmp'.format(unwrap_root)))
      
    def __getitem__(self, index):
        # 영상, 마스크 로드 및 크기 재설정
        gt_image = cv2.imread(self.paths[index])
        mask = cv2.imread(self.mask_paths[index])
        twodunwrap = cv2.imread(self.unwrap_paths[index])
        gt_image = cv2.resize(gt_image,(256,256))
        mask = cv2.resize(mask,(256,256))
        twodunwrap = cv2.resize(twodunwrap,(256,256))
        ...
        twodunwrap = Image.fromarray(twodunwrap)
        twodunwrap = self.img_transform(twodunwrap.convert('RGB'))
        
        return gt_img , maskk,twodunwrap, index
